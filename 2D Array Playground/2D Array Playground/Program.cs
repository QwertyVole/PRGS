...
namespace _2D_Array_Playground
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            foreach(int element in fiveByFive)
             {
                Random rnd = new Random();
                element = rnd.Next(1,26);
             }
            for(int i = 0; i <= fiveByFive.GetLength(0); i++) {
                for(int j = 0; j <= fiveByFive.GetLength(1); j++) {
                Console.Write(fiveByFive[i,j]);
}
            }
