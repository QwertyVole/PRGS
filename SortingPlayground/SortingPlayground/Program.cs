...
namespace SortingPlayground
{
    internal class Program
    {
        ...
        {
            int[] sortedArray = (int[])array.Clone(); // Řaď v tomto poli, ve kterém je výchoze zkopírováno všechno ze vstupního pole.
            int iMin = sortedArray[0];
            int cache;
            for(int iteration = 0; iteration < sortedArray.Length - 1; iteration++)
            {
                for(int i = iteration; i < sortedArray.Length - 1; i++)
                { 
                ...
                cache = sortedArray[iteration];
                sortedArray[iteration] = sortedArray[iMin];
                sortedArray[iMin] = cache;
                iMin = iteration;
            }
            return sortedArray;
        }
