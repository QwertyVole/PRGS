...
namespace SearchPlayground
{
    internal class Program
    {
        ...
        static string BinarySearch(int[] array, int elementToSearch)
        {
            int lower = 0;
            ...
            int steps = 0;
            Array.Sort(array);
            while(middle != elementToSearch) 
            { 
                middle = (lower + upper)/2;
                if(array[middle] == elementToSearch)
                    return $"{middle} v {steps} krocích";
        ...
        static void SearchArray(int[] array, string arrayName)
        {
            ...
            index = Convert.ToInt32(LinearSearch(array, randomElement));
            Console.WriteLine($"    Lineární vyhledávání našlo prvek {randomElement} na indexu {index}");

            output = BinarySearch(array, randomElement);
            Console.WriteLine($"    Binární vyhleádávání našlo prvek {randomElement} na indexu {output}");
            /*
